...
% 1. PROBLEM
\begin{frame}[shrink=12]{Problem \& contribution}
  ...
  \smallskip
  \alert{Users may not say ``grasp''---they might ask ``Where can I grasp this object?''
  How well does the model handle that change in wording?}

  \smallskip
  \textbf{Our approach} (pretrained weights, no new architecture):
  \begin{itemize}
  ...
  \begin{block}{Headline}
    Rephrasing costs \up{$\sim$12 mIoU}. Fine-tuning \up{101k params (5.7\%)} improves
    held-out questions by \up{11.2} and descriptions by \up{10.5 mIoU}.
  \end{block}
\end{frame}
...
\begin{frame}{Reproduction validates the harness}
  ...
  \begin{tabular}{@{}llccc@{}}
    ...
    \midrule
    Open-vocab & OpenAD (cited) & 14.37 & 46.31 & 19.51 \\
...
\begin{frame}{The problem: phrasing breaks the pretrained model}
  \begin{columns}[c]
    ...
    \footnotesize
    \begin{block}{Same intent, different wording}
      \begin{tabular}{@{}>{\bfseries}l@{\hspace{4pt}}p{0.72\linewidth}@{}}
        Canonical & \texttt{grasp} \\
        Question & ``Where should I take hold of this?'' \\
        Description & ``the surface meant for gripping by hand'' \\
      \end{tabular}
    \end{block}

    \medskip
    Rephrasing the queries as questions or descriptions reduces mIoU by about \up{12 points}.\\[5pt]
    Grasp, contain, and pourable fall close to zero, while \emph{cut} remains relatively strong.

    {\setbeamercolor{block title}{fg=black,bg=black!12}
    \setbeamercolor{block body}{fg=black,bg=black!4}
    \begin{block}{Useful geometry, weak text alignment}
      \scriptsize
      Across points: rephrased \texttt{grasp} scores handle $0.30 >$ body $0.10$\
      $\Rightarrow$ it still locates the handle region.\\[-1pt]
      At the handle: \texttt{grasp} $0.30 <$ \texttt{lift} $0.40$\
      $\Rightarrow$ another query wins the final class label.
    \end{block}}

    \alert{PointNet++ still distinguishes the useful region; its match to the rephrased text is too weak.}
  \end{columns}
\end{frame}

% 7. RESULTS
\begin{frame}{Results: fine-tuning improves held-out phrasing}
  \begin{columns}[T]
    ...
    \footnotesize
    \begin{itemize}
      \item Held-out questions improve by \up{11.2 mIoU}; descriptions improve by \up{10.5}.
      \item Held-out scores are close to seen scores, suggesting generalisation rather than exact phrase memorisation.
...
\begin{frame}{Ablation: how much of the network to train?}
  ...
  \begin{columns}[T]
    ...
    Values show the mIoU improvement after fine-tuning.\\
...
\begin{frame}{Conclusion}
  \begin{itemize}
    ...
    \item \up{Prompt-augmented fine-tuning} of 101k params (5.7\%) improves held-out questions
      by 11.2 and descriptions by 10.5 mIoU, while preserving canonical performance.
